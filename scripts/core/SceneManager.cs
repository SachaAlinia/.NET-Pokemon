...
public partial class SceneManager : Node
{
	...
	public static async void ChangeLevel(LevelName levelName = LevelName.map1, int trigger = 0, bool spawn = false)
	{	
		if (Instance == null)
		{
			// On attend un tout petit peu (une frame) pour laisser le temps au _Ready d'exécuter l'assignation
			await Task.Delay(1); 
			if (Instance == null) return; // Si c'est toujours null, il y a un gros souci de hiérarchie
		}
